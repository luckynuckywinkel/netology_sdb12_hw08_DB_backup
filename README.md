...
...и, не вдаваясь в подробности, по итогу я потратил несколько дней на изучения материала и завалил свой сервер MeSQL.  

Что я могу сказать. Percona XtraBackup - весьма мощное средство для бэкапировапния баз данных, но с ним нужно разбираться. К сожалению, я не могу привести вам скриншоты, т.к. в процессе изучения и выполнения разных действий (ошибок было много, а идеальнного результат сразу не выходило), я их не делал, надеясь-таки на некий идеальный результат.  

Но, вот, что я могу сказать по итогу. Полный и инкрементный бэкапы выше были мной сделаны правильно. Восстановление полного бэкапа происходит таким образом:  

```
# systemctl stop mysqld && rm -rf /var/lib/mysql/*
# xtrabackup --copy-back --target-dir=/home/winkel/backup
# chown -R mysql:mysql /var/lib/mysql
# systemctl start mysqld
```

То есть, мы просто удаляем содержимое каталога /var/lib/mysql/ и переносим туда наш полный бэкап, выдавая правильные права при этом.  

Инкрементальный бэкап делается следующим образом - мы подготавиливаем полную копию и добавляем туда инкремент (или инкременты) и далее восстанавливаем, как полный бэкап:  

```
# xtrabackup --prepare --apply-log-only --target-dir=/home/winkel/backup
# xtrabackup --prepare --apply-log-only --target-dir=/home/winkel/backup --incremental-dir=/home/winkel/inc_backup1
```

Здесь речь идёт о бэкапировании и восстановлении **всех** баз и всей структуры MySQL. Про бэкап и восстановление одной базы, пожалуй, писать не буду, ибо там все чуть сложнее.  

---  

Также, мы можем использовать двоичные файлы для восстановления инкрементных данных, эти файлы располагаются в каталоге /var/log/mysql, но такой подход я считаю не совсем правильным.
